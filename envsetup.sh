...
EOF
    ...
    __print_zirconium_functions_help
...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/zirconium/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/zirconium/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/zirconium/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^aosp_") ; then
        ZIRCONIUM_BUILD=$(echo -n $1 | sed -e 's/^zirconium_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $ZIRCONIUM_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        ZIRCONIUM_BUILD=
    fi
    export ZIRCONIUM_BUILD
...
. $ANDROID_BUILD_TOP/vendor/zirconium/build/envsetup.sh
